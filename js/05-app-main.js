...
function exportRasterWorldFile(entry){
  if(!entry || !entry.transform){
    alert('Esta imagem ainda não tem uma transformação calculada — não há World File para exportar.');
    return;
  }
...
/* converte os 6 parâmetros afins (a,b,c,d,e,f) na matriz 4x4 do
   ...
   NÃO USAR com geotiff.js 2.1.3 — ver nota em exportRasterGeoTiff. */
...
async function exportRasterImageZip(entry){
  ...
  if(typeof JSZip === 'undefined'){
    alert('A biblioteca de compressão (JSZip) não está disponível nesta página.');
    return;
  }
  let originalBytes;
  try{
    ...
    alert('Não foi possível preparar a imagem original para exportar.');
    return;
  }
  const baseName = fileBaseName(entry.source || entry.name || 'imagem');
...
async function exportRasterGeoTiff(entry){
  if(!entry || !entry.transform){
    alert('Esta imagem ainda não tem uma transformação calculada — não há GeoTIFF para exportar.');
    return;
  }
  if(typeof GeoTIFF === 'undefined' || typeof GeoTIFF.writeArrayBuffer !== 'function'){
    alert('Esta versão da biblioteca GeoTIFF (geotiff.js) não suporta exportação. Atualiza o script no engenh.html para uma versão mais recente.');
    return;
  }
  /* NOTA (bug confirmado na fonte de geotiff@2.1.3, dist-module/geotiffwriter.js):
     se não passarmos `ModelPixelScale` explicitamente, o writer injeta sempre
     um valor por omissão que assume que o raster cobre o globo inteiro
     (`[360/width, 180/height, 0]`) — mesmo que já tenhamos passado
     `ModelTransformation`. O ficheiro final fica com DUAS tags de
     georreferenciação em conflito, e o ArcGIS/GDAL acaba a usar a errada,
     daí a imagem aparecer esticada pelo mundo todo a partir de (0,0).
     Por isso usamos aqui ModelPixelScale+ModelTiepoint (que este writer
     respeita corretamente) em vez de ModelTransformation, o que só cobre
     o caso sem rotação/shear — o mais comum. Se a transformação tiver
     rotação real, recusamos o GeoTIFF direto e sugerimos o World File/ZIP,
     que não passam por este writer e por isso não têm este problema. */
  if(affineHasRotation(entry.transform)){
    alert('Esta imagem tem uma transformação com rotação, que o exportador GeoTIFF atual ainda não consegue representar sem erros de georreferenciação. Usa a opção "Imagem + World File (.zip)" em alternativa — essa preserva a rotação corretamente.');
    return;
  }
  ...
  try{
    ...
    alert('Não foi possível ler os pixels da imagem para gerar o GeoTIFF.');
    return;
  }

  const {data, width, height} = imageData;
  // deteta transparência real: se não houver nenhum pixel com alfa < 255,
  // exporta só RGB (ficheiro mais pequeno e mais compatível)
  let hasAlpha = false;
  ...
  try{
    ...
    alert('Não foi possível gerar o GeoTIFF: ' + err.message);
    return;
  }
...
function applyGeorefTransform(){
  ...
  try{
    ...
    alert('Não foi possível calcular a transformação: ' + err.message);
    return;
  }
...
async function fetchDgtReferenceTile(z, x, y){
  ...
  try{
    ...
    const bounds = parseWmsTileBoundsFromUrl(url);
...
/* Tile de referência alternativo: Satélite (Esri World Imagery), usado como
   fallback quando o DGT falha a carregar OU a deteção contra o DGT não
   produz um resultado fiável (ex.: a imagem a georreferenciar é um print do
   Google Maps/satélite, que se parece muito mais com esta camada do que com
   o ortofoto DGT — cores, ano de captura e ângulo diferentes).

   Ao contrário do WMS da DGT, o serviço ArcGIS World_Imagery já é usado
   diretamente no mapa (ver OfflineTileLayer, `satelite` em basemapLayers) e
   tipicamente envia Access-Control-Allow-Origin — por isso tenta-se sempre
   um fetch direto primeiro. Mantém-se o proxy genérico (/api/download) como
   rede de segurança, para o caso de a política de CORS mudar ou a rede do
   utilizador bloquear pedidos diretos ao arcgisonline.com. */
async function fetchSatelliteReferenceTile(z, x, y){
  ...
  async function fetchSingleSatelliteTile(tx, ty){
    ...
    try{
      ...
      response = await proxyFetchResource(url); // erro aqui propaga-se ao chamador (Promise.allSettled trata-o)
    }
    const contentType = response.headers.get('Content-Type') || '';
    if(contentType && !contentType.startsWith('image/')){
...
async function runAutoGeorefDetection(){
  if(!georefModeState.active || !georefModeState.entry){
    alert('Ativa o modo de georreferenciação antes de usar a autogeorreferenciar.');
    return;
  }
...
async function processImportedFiles(files){
  ...
  if(images.length > 0){
    try{
      ...
      alert('Não foi possível importar uma ou mais imagens. Verifica se os ficheiros não estão corrompidos.');
    }
  }
